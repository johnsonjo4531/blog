...
Well you can manually do each of the above everytime you need to speak any text or you could create a little function that
can be run from any terminal session. We'll use the bash_profile which runs each time your terminal is started.
The below function should be put into your `~/.bash_profile` file on your mac (if that file doesn't exist create it and paste this in there).

```bash
# #!/bin/bash

# Takes a text file (.txt,.epub,.mobi,etc.) and outputs the spoken text as an audio file
saymp3help () {
  echo 'saymp3 -i <INPUT_TEXT_FILE> -o <OUTPUT_AUDIO_FILE> -v <VOICE> [-s </regex to split on/>]
  -i=input file (any pandoc input file that can be output to text)
  -o=output file (an audio file type)
  -v=voice (use `say -v ?` for a list of voices)
  -s=split_regex the Regex to split by chapter with (default is /^Chapter.*\d.*/)
  '
}
...
if [[ $* != *-v* ]] && [[ $* != *-i* ]] && [[ $* != *-o* ]]; then
  echo "You must provide the -v, -i, and -o flags/options/arguments";
  saymp3help;
  exit 1;
fi

declare OPTIND option input output voice split_regex=/^Chapter.*\d.*/

# extract options and their arguments into variables.
while getopts "i:o:v:s:" option ; do
    case "${option}" in
        i)
            input=${OPTARG}  ;;
        o)
...
tempdir="$TMPDIR""saymp3"

echo "Converting to Markdown"
mkdir "$tempdir";
# Convert the input text file to markdown to get headings
pandoc -o "$tempdir/temp.md" "$input";
# gcsplit line and iteration logic were made by my Brother-in-Law Matthew Dayley.
# Split the markdown-file by chapter through a regex
echo "Splitting into Chapter files"
gcsplit -s -z -f "$tempdir/chapter" -b "%.3d" "$tempdir/temp.md" "$split_regex" {*};
# # https://stackoverflow.com/a/965072/2066736
filename=$(basename -- "$output");
extension="${filename##*.}";
outdir=$(dirname "$output")
files=$(for i in `ls "$tempdir/"chapter*`; do echo $(basename -- "$i"); done;);
chapters=`echo $(echo $files | wc -w)`
# # Loop through and modify 
for i in $files; do
  # Run the conversion from text/epub/etc. to mp3
  echo "Creating plain text for $i of $chapters:" && \
    pandoc -t plain -o "$tempdir/temp.txt" "$tempdir/$i" && \
    echo "Converting text to speech for $i of $chapters:" && \
    say -f "$tempdir/temp.txt" -v "$voice" -o "$tempdir/temp.aiff" && \
    echo "Converting aiff to other music file type for $i of $chapters:" && \
    ffmpeg -y -i "$tempdir/temp.aiff" "$outdir/audio_$i.$extension";
done;
```

Once that is in your profile either source your bash_profile with `source ~/.bash_profile` or just restart your terminal
and you should be able to use the function like so:

```
saymp3 -i <INPUT_TEXT_FILE> -o <OUTPUT_AUDIO_FILE> -v <VOICE>
```

Where `<INPUT_FILE>` is some .pub,.txt,.mobi,etc.
