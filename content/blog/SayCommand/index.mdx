...
We particularly want to useWhen you are done looking at the manual page hit `q` to close it.
...
by itself to see what that it outputs the file contents to stdout.
...
Pandoc's example is similar to the below command which converts a `.docx` file to `.txt`:
...
Similarly the output (signified by the `-o` flag)
...
if the previous command succeeded. The `\` allows us to escape the linebreaks so we can do multiple lines</aside>
...
sort of document in place of `input.docx` and you'll get an `output.mp3`.
